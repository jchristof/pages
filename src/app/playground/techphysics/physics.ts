...
declare var CANNON;
export class Physics implements IGame{
    ...
    var groundBody =  CANNON.RigidBody(0, groundShape, groundMaterial); 
...
// ground - heightmap
var ground =  BABYLON.Mesh.CreateGroundFromHeightMap("ground", "carx/heightmap.png", 700, 700, 50, 0, 25, scene, false);  
...
var water =  BABYLON.Mesh.CreatePlane("water", 95, scene);
...
var boxx = new Array(8);
    boxx[0] =  BABYLON.Mesh.CreateBox("boxx", 3, scene);
    boxx[0].scaling.y =    10;
    ...
    // bridge point 2
    boxx[7] = BABYLON.Mesh.CreateBox("boxx", 6, scene);
...
var wbox = new Array(wb.length);
    wbox[0] = BABYLON.Mesh.CreateBox("box1", 10, scene);
    wbox[0].material = materialbox;
for (var i = 1; i < wb.length; i++) {
    wbox[i] = BABYLON.Mesh.CreateBox("wbox1", 6, scene);
    wbox[i].material = materialbox4;
}

// bridge
var bridge1 = BABYLON.Mesh.CreateBox("bridge1", 1, scene);
...
var bord = new Array(bor.length);
    bord[0] = BABYLON.Mesh.CreateBox("bord1", 1, scene);
...
// ramp
var ramp1 = BABYLON.Mesh.CreateBox("ramp1", 1, scene);
...
// car chassis
var chassis = BABYLON.Mesh.CreateBox("chassis", 1, scene, false);
    chassis.visibility = 0;
var chassis_a = BABYLON.Mesh.CreateCylinder("chassis_a", 1, 1, 1, 30, 10, scene,false);
...
var chassis_b = BABYLON.Mesh.CreateBox("chassis_b", 1, scene, false);
...
for (var i = 0; i < xxtree-5; i++) {
    var tree = new BABYLON.Sprite("tree", spriteManagerTrees);
        ...
        tree.size = 20;
    // for collision
    treebox[i] = BABYLON.Mesh.CreateCylinder("treebox", 18, 0.7, 0.7, 20, 10, scene);
...
var norm = 1, up = 0, bridge_c = 0, ramp_c = false;
